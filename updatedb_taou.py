...
class Updatexag(CONNECTSQL):
    ...
    def get(self, count, tm):  # 1m,5m,15m,30m,60m qid: 6 agtd, 13 xag, 704 连续
        url = "https://official.gkoudai.com/officialNetworkApi/CandleStickV2?qid=13&type=" + str(
            self.type) + "&count=" + str(
            count) + "&ts=" + str(tm)
        header = {'epid': 'a6c89023-9472-4f30-81cf-8c7dea62aae5'}
        r = requests.post(url, headers=header)
...
class UpdateAny(CONNECTSQL):
    def __init__(self, table_name, qid, type, count=114):
        re_sq = 'select t from (select count(ts)t,ts from ' + table_name + ' group by ts)b where t>1'
        del_sq = 'delete from ' + table_name + ' order by ts desc limit 1'
        get_sq = 'select ts,t from ' + table_name + ' order by ts desc limit 1'
        self.updata_sq = 'insert into ' + table_name + ' (a,c,t,v,h,l,o,ts) values(%s,%s,%s,%s,%s,%s,%s,%s)'
    ...
    # type1, 2, 3,  4,  5:
    def get(self, qid, count, tm):  # 1m,5m,15m,30m,60m qid: 6 agtd, 13 xag, 704 连续,300 usd-rmb ,102 xau
        url = "https://official.gkoudai.com/officialNetworkApi/CandleStickV2?qid=" + str(qid) + "&type=" + str(
            self.type) + "&count=" + str(
            count) + "&ts=" + str(tm)
        header = {'epid': 'a6c89023-9472-4f30-81cf-8c7dea62aae5'}
        r = requests.post(url, headers=header)
    ...
    def updatedb(self, mydb):
        ...
        try:
            for i in range(0, 30):
                ...
                if tm <= 1533566880000 - 1:
                    break
                rev = self.get(self.qid, self.count, tm)
                print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(rev[0][-1][-1] / 1000)),
                      time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(self.lasttime / 1000)))
                # rev[0].reverse()
                for i in range(len(rev[0])):
...
if __name__ == '__main__':
    ...
    con_sql = UpdateAny('usd_rmb',300, 5, 120)
    con_sql.updatedb(con_sql.mydb)
    print('usd_rmb_over')

    con_sql = UpdateAny('xau1h',120, 5, 120)
    con_sql.updatedb(con_sql.mydb)
    print('au1h_over')