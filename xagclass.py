...
class GetXag():
    ...
    # main
    def ot(self, bei, xie, late, o, c, h, l, ts, c1,ma_range):  # 主策略
                ...
                if sell == 1:  # 平空
                    ...
                    ee.append(
                        [float(e), float(Dec(str(so)) - Dec(str(sc))), '空平', sc, ma20[i], o[i], c[i], h[i], l[i],
                         time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(ts[i] / 1000)), i, '', chicang,(float(Dec(str(so)) - Dec(str(sc))))/so,CIrate])
                    # print(e, '空平',sc,ts[i],si,sign)
                    # print(bo, bc, so, sc, si, sign)
                    lates, sell, chicang, maxh, minl = 0, 0, 0, 0, 10000
                bct = 1
                # 开多,low<19收+1开的ma,前1ma>=前2ma
                if l[i] <= (round(ma20[i], 3) - round((c[i] - o[i]) / 20, 3)) and buy == 0 and (1 < lateb <= late) and \
                        ma20[
                            i] >= ma20[i - 1]:
                    if round(ma20[i], 3) > o[i]:  # 低于ma开盘开仓
                        ...
                        bo = math.ceil(round(ma20[i], 3) - round((c[i] - o[i]) / 20, 3))  # 19收+1开的ma价挂单开仓
                    tskc = ts[i]
                ...
                if buy == 1:
                    ...
                    ee.append(
                        [float(e), float(Dec(str(bc)) - Dec(str(bo))), '多平', bc, ma20[i], o[i], c[i], h[i], l[i],
                         time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(ts[i] / 1000)), i, '', chicang,(float(Dec(str(bc)) - Dec(str(bo))))/bo,CIrate])
                    # print(e, '多平', bc,ts[i],si,sign)
                    # print(bo, bc, so, sc)
                    # input()
                    lateb, buy, chicang, maxh, minl = 0, 0, 0, 0, 10000
                sct = 1
                if h[i] >= (round(ma20[i], 3) - round((c[i] - o[i]) / 20, 3)) and sell == 0 and (1 < lates <= late) and \
                        ma20[i] <= ma20[i - 1]:
                    if round(ma20[i], 3) < o[i]:
                        ...
                        so = math.floor(round(ma20[i], 3) - round((c[i] - o[i]) / 20, 3))
                    tskc = ts[i]
