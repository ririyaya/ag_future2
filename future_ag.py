...
for ma_range in range(24, 25):
    f_ag = trend.Fag(2, ma_range, 'f_ag_15m')  # 杠杆倍率,表,强制循环起点
    for slope in range(4, 5):
        for maxlate in range(9, 10):  # 斜率?
            for minlate in range(2, 3):  # startlate
                fag, log, CIrate = f_ag.ot(100000, slope, ma_range, minlate, maxlate, f_ag.o, f_ag.h, f_ag.l, f_ag.ts, f_ag.c1, f_ag.ma)
                ...
                print(tex)
                explain.append(trend.plt_draw(fag, ma_range))
